"""
Call AgentCore Runtime (invoke_agent_runtime) directly. No streaming server.
Run Streamlit from repo root so .bedrock_agentcore.yaml is found; AWS credentials (e.g. aws configure) required.
"""
import json
import logging
import os
...
def invoke_stream_direct(
    ...
    if not config_path:
        logger.error("config not found; run from repo root or set AGENT_CONFIG_PATH")
    ...
    try:
        ...
        logger.exception("config load failed path=%s", config_path)
        yield f"Config error: {e}"
        return

    default = config.get("default_agent")
    if not default or default not in config.get("agents", {}):
        logger.error("default_agent not found in .bedrock_agentcore.yaml")
    ...
    if not arn:
        logger.error("no agent_arn in config; run agentcore deploy")
        yield "No agent_arn. Run: agentcore deploy"
        return

    logger.info(
        "invoke_stream_direct start session_id=%s prompt_len=%d region=%s",
        session_id,
        len(prompt or ""),
        region,
    )
    ...
    try:
        ...
        logger.exception("invoke_agent_runtime failed session_id=%s", session_id)
        yield f"Runtime invoke failed: {e}"
        return
    ...
    try:
        ...
        logger.info("invoke_stream_direct done session_id=%s (json response len=%d)", session_id, len(out))
        yield result if isinstance(result, str) else json.dumps(data)
    except json.JSONDecodeError:
        logger.warning("invoke_stream_direct non-json response session_id=%s len=%d", session_id, len(out))
        yield out
